...
class MetricName(CapeEnum):
    ...
    COUNT_ITERS_KIT = 'Count[Iter]_kIt'
...
class NonMetricName(CapeEnum):
    ...
    SI_TIER_NORMALIZED = "Normalized_Tier"
...
SUMMARY_METRICS = RUN_INFO_METRICS + TIME_METRICS + COUNT_METRICS + RATE_METRICS \
    + BRANCH_METRICS + STALL_METRICS + MEM_ACCESS_METRICS + WHATIF_SPEEDUP_METRICS + ENERGY_METRICS + SRC_METRICS

CAPACITY_METRICS = [ m for m in ALL_METRICS if m.startswith("C_") ]

SIDO_CLUSTER_METRICS=[ NonMetricName.SI_CLUSTER_NAME, NonMetricName.SI_SAT_NODES, NonMetricName.SI_SAT_TIER, NonMetricName.SI_SW_BIAS, NonMetricName.SI_TIER_NORMALIZED ]
