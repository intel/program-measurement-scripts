...
build_codelet () {
	...
	build_folder="$3"
        ...
        echo "Codelet_folder: $codelet_folder"
#read -p "Press [Enter] key to continue..."
	...
	if [[ "$ENABLE_SEP" == "1" ]]; then
		...
		make LIBPATH="${BASE_PROBE_FOLDER}" SRC_DIR="$codelet_folder" clean all
	fi
...
run() {
	...
	lore_prefix="${prefix}/src"
	lore_xformed_prefix="${lore_prefix}/xformed"
	lore_all_prefix="${lore_prefix}/all"
	...
	fill_codelet_maps "${lore_all_prefix}" "${linear_sizes}"
	...
	run_codelets=(
#deriche.c_init_array_line35_jl0f2_tl1s256_tl2s256
                # Accept from environmental variables
                $PGMS
	)



	name2sizes[deriche.c_init_array_line35_jl0f2_tl1s256_tl2s256]="8000"
	...
	runId="${runId}" variants="$variants" memory_loads="$memory_loads" frequencies="$frequencies"  num_cores="$num_cores" prefetchers="$prefetchers" counter_list_override="RESOURCE=1,SQ=0,SQ_HISTOGRAM=0,LFB_HISTOGRAM=0,TOPDOWN=0,LFB=0,MEM_ROWBUFF=0,MEM_TRAFFIC=1,MEM_HIT=1,TLB=0,LSD=0,FLOP=1" runLoop
