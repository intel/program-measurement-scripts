...
#	echo $new_set
	event_set_counters=$event_set_counters";"$new_set
done
event_set_counters=$(echo $event_set_counters | sed 's:^;::')

#Building the core events list
fuse_core_events=$(echo $event_set_counters | tr ';' '\n' | egrep "^Event_Set" | tr '\n' ';')
fuse_core_events=$(echo $fuse_core_events | sed 's:;$::')
#echo $fuse_core_events

#Building the uncore events list
uncore_events=$(echo $event_set_counters | tr ';' '\n' | grep "Uncore" | tr '\n' ';')
uncore_events=$(echo $uncore_events | sed 's:;$::')
uncore_sets=$(echo $event_set_counters | tr ';' '\n' | grep "Uncore" |  cut -d':' -f1 | sed 's:.*\(Event_Set_.*\):\1:' | sort -u )
fuse_uncore_events=""
for s in $uncore_sets
do
	myset=$(echo $uncore_events | tr ';' '\n' | grep $s | cut -d':' -f2 | tr '\n' ',')
    myset=$(echo $myset | sed 's:,$::')	
	fuse_uncore_events="$fuse_uncore_events;""Uncore_$s:$myset"
done
fuse_uncore_events=$(echo $fuse_uncore_events | sed 's:^;::')
